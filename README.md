...
That run is an earlier capture. Every other run was re-captured after the pipeline
gained identity resolution and `corpus_search`, and all of them pass — furA included,
so the re-run sits next to the original: the same gene, once wrong and once right.
